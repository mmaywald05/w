...
__global__ void fftKernel(float *real, float *imag, int n, int step) {
    ...
    while (m >= step) {
        ...
        float tempImag = sinf(angle) * real[k + m] + cosAngle * imag[k + m];
...
__global__ void magnitudeKernel(float *real, float *imag, float *magnitudes, int n) {
    int tid = blockIdx.x * blockDim.x + threadIdx.x;
    if (tid < n) {
        magnitudes[tid] = sqrtf(real[tid] * real[tid] + imag[tid] * imag[tid]);
    }
}

void cudaFFT(float *real, float *imag, float *magnitudes, int n, int blockSize, int shift) {
    int bits = log2f(n);
    float *d_real, *d_imag, *d_magnitudes;
    cudaMalloc(&d_real, n * sizeof(float));
    cudaMalloc(&d_imag, n * sizeof(float));
    cudaMalloc(&d_magnitudes, n * sizeof(float));
    ...
    magnitudeKernel<<<grid, block>>>(d_real, d_imag, d_magnitudes, n);
    cudaDeviceSynchronize();

    cudaMemcpy(magnitudes, d_magnitudes, n * sizeof(float), cudaMemcpyDeviceToHost);
    cudaFree(d_real);
    cudaFree(d_imag);
...
int main(int argc, char *argv[]) {
    ...
    std::vector<float> magnitudes(k, 0.0f);
    std::vector<float> totalMagnitudes(k, 0.0f);

    for (int i = 0; i <= n - k; i += s) {
        cudaFFT(real.data() + i, imag.data() + i, magnitudes.data(), k, 256, s);
        for (int j = 0; j < k; ++j) {
            totalMagnitudes[j] += magnitudes[j];
        }
    }

    std::vector<float> averageMagnitudes(k);
    for (int i = 0; i < k; ++i) {
        averageMagnitudes[i] = totalMagnitudes[i]/numBlocks;
    }

    for (int i = 0; i < k; ++i) {
        std::cout << "Frequency bin " << i << ": average magnitude = " << averageMagnitudes[i] << std::endl;
    }
